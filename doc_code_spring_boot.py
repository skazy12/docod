...
def es_archivo_importante(nombre_archivo):
    extensiones_importantes = [
        '.java', '.html', '.properties', '.yml', '.yaml', '.xml', '.groovy', '.gradle', '.json'
    ]
    return os.path.splitext(nombre_archivo)[1].lower() in extensiones_importantes

def es_archivo_thymeleaf(nombre_archivo):
    return nombre_archivo.endswith('.html') and 'templates' in os.path.dirname(nombre_archivo).lower()
    ...
    with open(archivo_salida, 'w', encoding='utf-8') as archivo_txt:
        for root, dirs, files in os.walk(directorio_a_documentar):
            ...
            for file in files:
                ...
                if es_archivo_importante(file) or es_archivo_thymeleaf(os.path.join(root, file)):
